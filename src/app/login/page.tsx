// ===== OptiTalk - Login/Register Page =====
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { toast } from 'sonner';
import { Mail, Phone, Lock, User as UserIcon, Loader2, LogIn, UserPlus, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(mode === 'login' ? 'أهلاً بعودتك! 👋' : 'تم التسجيل بنجاح! 🎉');
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col items-center justify-center bg-[#0a0e1a] text-white px-6 py-10 overflow-hidden">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-20 -right-20 h-72 w-72 rounded-full opacity-30 blur-3xl" style={{ background: 'radial-gradient(circle, #6C5CE7 0%, transparent 70%)' }} />
        <div className="absolute -bottom-20 -left-20 h-80 w-80 rounded-full opacity-25 blur-3xl" style={{ background: 'radial-gradient(circle, #00CEC9 0%, transparent 70%)' }} />
        <div className="absolute top-1/3 left-1/4 h-48 w-48 rounded-full opacity-15 blur-3xl" style={{ background: 'radial-gradient(circle, #D4A03C 0%, transparent 70%)' }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-md space-y-6">
        <div className="flex flex-col items-center gap-3">
          <div className="relative flex h-24 w-24 items-center justify-center">
            ...
            <img src="/logo.png" alt="OptiTalk" className="relative h-20 w-20 rounded-[1.5rem] object-cover" />
          </div>
          <h1 className="text-3xl font-black tracking-tight">
            <span style={{ background: 'linear-gradient(to right, #6C5CE7, #00CEC9, #D4A03C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              OptiTalk
            </span>
          </h1>
        ...
        <div className="flex p-1 bg-white/5 rounded-2xl border border-white/10">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all ${mode === 'login' ? 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-lg' : 'text-white/60 hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all ${mode === 'register' ? 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-lg' : 'text-white/60 hover:text-white'}`}
          >
        ...
        <button
          ...
          className="w-full text-center text-xs text-white/40 hover:text-white/70 pt-2"
        >
          ← الرجوع
        </button>
      </div>
